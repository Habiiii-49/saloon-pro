...
$rows  = financeRevenueGroup($db, $from, $to, 'service');
...
if (isset($_GET['export'])) {
    ...
    header('Content-Disposition: attachment; filename="service-revenue-' . $from . '-to-' . $to . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['Service', 'Revenue', 'Share %']);
    foreach ($rows as $r) {
        fputcsv($out, [$r['label'], number_format((float)$r['value'], 2, '.', ''), $total > 0 ? number_format(((float)$r['value'] / $total) * 100, 1) : '0']);
    }
    fputcsv($out, ['TOTAL', number_format($total, 2, '.', ''), '100']);
    fclose($out);
    exit;
}
?>
...
<div class="page-head">
    <div class="page-head-title">
        <h1><i class="fas fa-scissors"></i> Revenue by Service</h1>
        <p>Revenue is allocated pro-rata to each invoice's line items.</p>
    </div>
...
<div class="finance-grid mb-4">
    <div class="stat-card"><div class="stat-value"><?php echo formatMoney($total); ?></div><div class="stat-label">Allocated Revenue</div></div>
    <div class="stat-card"><div class="stat-value"><?php echo formatMoney($net); ?></div><div class="stat-label">Net Collected</div></div>
</div>

<div class="admin-card">
    <div class="card-header-custom"><h5><i class="fas fa-ranking-star"></i> Services by Revenue</h5></div>
    <div class="table-scroll">
        <table class="admin-table">
            <thead><tr><th>#</th><th>Service</th><th class="fin-amount">Revenue</th><th>Share</th></tr></thead>
            <tbody>
                <?php
                $i = 1;
                foreach ($rows as $r): ?>
